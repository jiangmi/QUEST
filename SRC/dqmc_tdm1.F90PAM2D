...
!--------------------------------------------------------------------!
  ...
  subroutine DQMC_TDM1_Print1(T1, OPT1, OPT2, OPT3, OPT4)
    use dqmc_mpi
    !
    ! Purpose
    ! =======
    ...
    !
    type(TDM1), intent(in)   :: T1                 ! T1
    integer, intent(in)      :: OPT1, OPT2, OPT3, OPT4
    ! OPT4 records the results:
    ! S+S-(beta/2) for cc, cf, ff in PAM model
    ! also Chi_cc, Chi_cf, Chi_ff 
  ...
  ! both SPXX (for 1/T1T) and SPZZ (to average with SPXX for accuracy)
    ...
    ! onsite or c-f spin correlation
      if (index(title, " 0.0000  0.0000") > 0) then
        call DQMC_Print_Array(0, T1%L, label, tmp(:, 1:1), tmp(:, 2:2), OPT1)
        
        ! ONLY print out values at tau = beta/2 
        call DQMC_Print_Array(T1%L, label, tmp(:, 1:1), tmp(:, 2:2), OPT4)
      endif
    !  write(OPT1,'(1x)')
    enddo
  ...
  ! G(tau) for conduction electron and f electron seperately
    do i = 1, T1%properties(IGFUN)%nclass
      ...
      if (index(title, "Gfun         0  0.0000  0.0000") > 0) then
        call DQMC_Print_Array(0, T1%L, label, tmp(:, 1:1), tmp(:, 2:2), OPT2)
      endif
      if (index(title, "Gfun         1  0.0000  0.0000") > 0) then
        call DQMC_Print_Array(0, T1%L, label, tmp(:, 1:1), tmp(:, 2:2), OPT3)
      endif
    enddo
 ...
 !--------------------------------------------------------------------!
  ...
  subroutine DQMC_TDM1_Chi_Print(T1, OPT, OPT1)
    use dqmc_mpi
    !
    ! Purpose
    ! =======
    ...
    character(len=30)   :: labeltemp
    real                :: nnn
    real                :: a(T1%properties(ISPXX)%nclass,4)
    integer             :: F(T1%properties(ISPXX)%nclass)
    real*8              :: chicc=0.0, chicf=0.0, chiff=0.0
   ...
   ! inv_[0,beta] includes value at beta
 !   write(OPT,*) "include value at beta"
 !   do iprop = ISPXX, ISPZZ
 !     do i = 1, nn
 !        tmp(i, 1) = sum(T1%properties(iprop)%values(i, 0:T1%L-1, T1%avg)) &
 !                    + T1%properties(iprop)%values(i, 0, T1%avg)
 !        ! Note that tmp(i,2) is in fact wrong (it is sum of error)
 !        tmp(i, 2) = sum(T1%properties(iprop)%values(i, 0:T1%L-1, T1%err)) &
 !                    + T1%properties(iprop)%values(i, 0, T1%err)
 !     enddo
 !     title = pname(iprop)
 !     call DQMC_Print_RealArray(0, nn, title, label, tmp(:, 1:1), tmp(:, 2:2),
 !     OPT)
 !     write(OPT,'(1x)')
 !   enddo

  ! ------------------------------------------------------------
  ! Calculate Chicc, Chicf, Chiff for both XX and ZZ
  nnn = sqrt(real(T1%properties(ISPXX)%n/2))  ! linear lattice size,/2 is for bilayer
write(OPT,*) "n=",T1%properties(ISPXX)%n
write(OPT,*) "nnn=",nnn

  ! First get the frequency for each class seperation
    do i = 1, nn
       ! Convert string to four real numbers:
       labeltemp = label(i)
       read(labeltemp(1:3)  ,*) a(i,1)
       read(labeltemp(4:11) ,*) a(i,2)
       read(labeltemp(12:19),*) a(i,3)
       read(labeltemp(20:27),*) a(i,4)
write(OPT,*) a(i,1)
write(OPT,*) a(i,2)
write(OPT,*) a(i,3)
write(OPT,*) a(i,4)

       if ((a(i,2)==0.0 .and. a(i,3)==0.0) .or. (a(i,2)==nnn*0.5 .and. a(i,3)==nnn*0.5)) then
         F(i) = 1
       else if ((a(i,2)==0.0 .and. a(i,3)==nnn*0.5) .or. (a(i,2)==nnn*0.5 .and. a(i,3)==0.0)) then
         F(i) = 2
       else if (a(i,2)==0.0 .or. a(i,3)==0.0) then
         F(i) = 4
       else if (a(i,2)==nnn*0.5 .or. a(i,3)==nnn*0.5) then
         F(i) = 4
...
write(OPT,*) "F",F(i)
   ...
   ! inv_[0,beta] NOT includes value at beta
 !   write(OPT,*) "NOT include value at beta"
    do iprop = ISPXX,ISPZZ
      ...
      ! Output the data, not related to Chi calculation
      do i = 1, nn
         tmp(i, 1) = sum(T1%properties(iprop)%values(i, 0:T1%L-1, T1%avg))
         ! Note that tmp(i,2) is in fact wrong (it is sum of error)
         tmp(i, 2) = sum(T1%properties(iprop)%values(i, 0:T1%L-1, T1%err))
    ...
    do i = 1, nn
      if (a(i,1)==0.0 .and. a(i,4)==0.0) then
        chicc = chicc + tmp(i,1)*F(i)
      else if (a(i,1)==0.0 .and. a(i,4)==1.0) then
        chicf = chicf + tmp(i,1)*F(i)
      else if (a(i,1)==1.0 .and. a(i,4)==0.0) then
        chiff = chiff + tmp(i,1)*F(i)
      endif
    enddo
 
    ! Notice: chi's should be multiplied by dtau finally 

    write(OPT,'(F16.9)') chicc
    write(OPT,'(F16.9)') chicf
    write(OPT,'(F16.9)') chiff

    ! Print into file "results"
    write(OPT1,'(F16.9)') chicc
    write(OPT1,'(F16.9)') chicf
    write(OPT1,'(F16.9)') chiff

   enddo ! end of iprop loop
